...
export default {
  ...
  methods: {
    ...
    handleOkASIC () {
      ...
      if (this.isAdd) {
        requestParams.jsonString.regulator = this.searchForm.regulator
      }
      requestParams.jsonString.ibAccounts.forEach((item,index)=>{
        if (item.account===''||item.metaTraderServerType===undefined) {
          requestParams.jsonString.ibAccounts.splice(index,1)
        }
      })
      this.formData.append('jsonString',JSON.stringify(requestParams.jsonString))
      this.confirmLoadingASIC = true
      if (this.isAdd) {
        getSpecialTableListAdd(Object.assign(this.formData,this.searchForm)).then(res => {
          if (res.code === 200) {
        ...
        getSpecialTableListUpdate(this.formData).then(res => {
          if (res.code === 200) {
            this.confirmLoadingASIC = false
            this.$message.success(res.msg)
            this.getASICData({regulator: 'ASIC'})
          ...
          return new Promise((resolve, reject) => {
            getSpecialTableListDel(record.id).then((res) => {
              if (res.code===200) {
                ...
                that.getASICData({regulator: 'St. Vincent'})
                resolve();
              } else {
